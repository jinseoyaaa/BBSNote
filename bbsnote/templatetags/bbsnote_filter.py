...
만약 게시물 전체 건수가 12개라면 첫번째 페이지는 번호가 12~3까지 역순으로 보여지고, 두번째 페이지에는 2~1까지 보여져야 한다.
...
따라서 전체 게시물 개수가 12개이고 페이지당 10건씩 게시물을 보여준다면 공식에 의해 1페이지의 번호는 12-1-(0~9 반복)+1이 되어 12~3까지 표시될 것이다.
2페이지의 경우에는 12-11-(0~1 반복)+1이 되어 2~1이 표시될 것이다.

템플릿에서 이 공식을 적용하려면 빼기 기능이 필요하다.
앞에서 더하기 필터(|add:5)를 사용한 것처럼 빼기 필터(|sub:3)가 있으면 좋을 것 같다.
하지만 장고에는 빼기 필터가 없다. 그래서 우리는 빼기 필터를 직접 만들 것이다.
|add:-3과 같이 숫자를 직접 입력하면 더하기 필터를 이용하여 원하는 값을 뺀 결과를 화면에 보여줄 수 있다.

하지만 이 방법은 이곳에서 사용할 수 없다.
